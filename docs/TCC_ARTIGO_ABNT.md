...
*A developer-oriented web application security analysis platform: finding explainability and noise reduction in non-destructive dynamic testing*

Ricardo José da Silva[^1]

[^1]: Acadêmico do curso de Engenharia de Software, Centro Universitário Leonardo da Vinci (UNIASSELVI). Turma EGS0011 — Experiência Profissional (TCC). E-mail institucional do autor. Orientador(a): [inserir nome].

---

## RESUMO

A avaliação de segurança de aplicações web frequentemente ocorre de forma tardia e pontual em equipes de desenvolvimento que não dispõem de um profissional dedicado à segurança da informação, o que permite que vulnerabilidades permaneçam desconhecidas até etapas avançadas do ciclo de vida do software, elevando o custo de correção. Ferramentas automatizadas de teste dinâmico (DAST) existem há décadas, mas a literatura registra que desenvolvedores as subutilizam por produzirem grande volume de resultados de difícil interpretação, com muitos falsos positivos e baixa orientação à correção. Este trabalho apresenta o SentinelScope, uma plataforma web de apoio à análise de segurança concebida sob a ótica do desenvolvedor, cujo objetivo é identificar possíveis vulnerabilidades e apresentar cada achado com severidade, confiança, evidência e orientação prática de correção, de modo verificável e não destrutivo. A solução foi construída como uma aplicação PHP sem framework, apoiada por um motor de varredura em Python organizado em camadas, com plugins passivos (34 verificações de transporte, cabeçalhos, cookies, CORS, cache, exposição de informação e de segredos) e plugins ativos seguros (injeção de SQL, XSS refletido, travessia de diretório, redirecionamento aberto, injeção de comando, SSTI, XXE, CRLF, host header, adulteração de verbo HTTP, injeção NoSQL, CORS e SSRF), todos submetidos a salvaguardas de escopo, orçamento de requisições e negação de alvos privados. Um diferencial central é a separação explícita entre *severidade* (impacto potencial) e *confiança* (força da evidência), além de uma camada de explicabilidade que traduz cada observação técnica em impacto para a aplicação e recomendação de correção em linguagem acessível, acompanhada de um exemplo de código "vulnerável → corrigido". A ferramenta ainda testa campos de formulário via POST e alcança áreas autenticadas por cabeçalho de sessão. A verificação foi conduzida por meio de uma suíte de 94 testes automatizados que exercitam cada plugin contra fixtures vulneráveis e seguros usando um servidor HTTP local real. Conclui-se que uma ferramenta de apoio orientada à explicabilidade e à redução de ruído é tecnicamente viável no escopo acadêmico e endereça uma lacuna reconhecida entre a capacidade de detecção e a capacidade de ação do desenvolvedor; propõe-se, como continuidade, uma avaliação empírica comparativa contra aplicações deliberadamente vulneráveis.
...
Security assessment of web applications is often performed late and sporadically in development teams that lack a dedicated information-security professional, allowing vulnerabilities to remain unknown until advanced stages of the software life cycle and raising remediation costs. Dynamic Application Security Testing (DAST) tools have existed for decades, yet the literature reports that developers underuse them because they produce large volumes of hard-to-interpret results, many false positives, and little remediation guidance. This work presents SentinelScope, a developer-centered web security analysis support platform whose goal is to identify possible vulnerabilities and present each finding with severity, confidence, evidence, and practical remediation guidance, in a verifiable and non-destructive manner. The solution was built as a framework-less PHP application supported by a layered Python scanning engine, with passive plugins (34 checks over transport, headers, cookies, CORS, cache, information and secrets exposure) and safe-active plugins (SQL injection, reflected XSS, path traversal, open redirect, command injection, SSTI, XXE, CRLF, host header, HTTP verb tampering, NoSQL injection, CORS and SSRF), all subject to scope enforcement, request budgets, and private-target denial. A central contribution is the explicit separation between *severity* (potential impact) and *confidence* (evidence strength), plus an explainability layer that translates each technical observation into application impact and a remediation recommendation in accessible language, accompanied by a concrete "vulnerable → fixed" code example. The tool also tests form fields via POST and reaches authenticated areas through a session header. Verification was conducted through a suite of 94 automated tests exercising each plugin against vulnerable and safe fixtures on a real local HTTP server. We conclude that a support tool oriented to explainability and noise reduction is technically feasible within an academic scope and addresses a recognized gap between detection capability and the developer's ability to act; a comparative empirical evaluation against deliberately vulnerable applications is proposed as future work.

**Keywords:** web application security; dynamic application security testing (DAST); OWASP Top 10; security tooling usability; DevSecOps.

---

## 1 INTRODUÇÃO

O software tornou-se o principal meio pelo qual organizações prestam serviços, e a aplicação web permanece uma das superfícies de ataque mais expostas, por ser acessível pela rede pública e por concentrar dados sensíveis. À medida que o número de aplicações mantidas por uma mesma equipe cresce, cresce também a dificuldade de garantir que cada nova versão ou manutenção não introduza falhas de segurança. Em equipes que não contam com um profissional dedicado exclusivamente à segurança da informação — cenário comum em pequenas e médias operações de desenvolvimento —, as verificações de segurança tendem a ocorrer de maneira pontual e não a cada entrega, permitindo que vulnerabilidades permaneçam desconhecidas até etapas posteriores do ciclo de vida do software (McGRAW, 2006).

Esse adiamento tem custo. É princípio consolidado da engenharia de software que o custo de corrigir um defeito cresce à medida que ele avança pelas fases do ciclo de desenvolvimento, sendo substancialmente maior quando descoberto em produção do que quando detectado durante a construção (BOEHM, 1981). No caso específico da segurança, o defeito tardio não implica apenas retrabalho: implica exposição indevida de informação, correções emergenciais e risco de incidente. Daí decorre o movimento contemporâneo conhecido como *shift-left*, que propõe antecipar as atividades de verificação de segurança para as fases iniciais e recorrentes do desenvolvimento, aproximando-as da rotina de quem escreve o código (McGRAW, 2006; SHOSTACK, 2014).

A automação é o instrumento natural dessa antecipação, e as ferramentas de *Dynamic Application Security Testing* (DAST) — que analisam a aplicação em execução, sem acesso ao código-fonte — existem há décadas. Entretanto, a literatura empírica é consistente ao apontar duas limitações que reduzem sua adoção efetiva pelos desenvolvedores. A primeira é de eficácia: estudos independentes demonstram que scanners de caixa-preta divergem significativamente entre si e deixam de detectar classes inteiras de vulnerabilidade, ao mesmo tempo em que geram falsos positivos (DOUPÉ; COVA; VIGNA, 2010; BAU et al., 2010; FONSECA; VIEIRA; MADEIRA, 2007). A segunda, e mais determinante para este trabalho, é de usabilidade: mesmo quando a ferramenta acerta, seus resultados são frequentemente apresentados em uma linguagem técnica orientada ao especialista em segurança, sem explicar *por que* aquilo importa para a aplicação específica e *como* corrigir no código. Johnson et al. (2013), ao investigarem por que desenvolvedores não usam ferramentas de análise estática, identificam justamente o excesso de falsos positivos e a apresentação inadequada dos resultados como barreiras centrais; Green e Smith (2016) argumentam que ferramentas e APIs de segurança precisam ser projetadas para o desenvolvedor comum, e não para o especialista, sob pena de serem ignoradas.
...
Como oferecer a equipes de desenvolvimento sem especialista dedicado uma avaliação inicial de segurança de aplicações web que seja, simultaneamente, automatizada, não destrutiva, incorporável à rotina e — sobretudo — apresentada de forma que o próprio desenvolvedor compreenda o risco e consiga corrigi-lo?
...
A relevância do tema decorre da conjunção entre a criticidade das vulnerabilidades web — sistematicamente catalogadas pelo OWASP Top 10 (OWASP, 2021) — e a subutilização documentada das ferramentas que deveriam mitigá-las (JOHNSON et al., 2013; GREEN; SMITH, 2016). Ao deslocar o foco da mera detecção para a *explicabilidade e acionabilidade* do achado, o trabalho posiciona-se sobre uma lacuna reconhecida na literatura e de valor prático direto para o cenário descrito no termo de referência do projeto: uma equipe que mantém mais de cinco aplicações internas sem apoio especializado em segurança. Academicamente, o trabalho articula três corpos de conhecimento — teste de segurança automatizado, taxonomias de vulnerabilidade e usabilidade de ferramentas de segurança — em um artefato verificável.

### 1.4 Delimitação do escopo

Trata-se de uma ferramenta de **apoio**, e não de substituição de auditoria especializada ou teste de intrusão profissional. Deliberadamente, exclui-se a exploração automática ou destrutiva das vulnerabilidades (a proposta é comprovar a existência do problema com a evidência mínima necessária, sem comprometer disponibilidade, integridade ou dados), a análise de código-fonte (a solução não pressupõe acesso ao repositório) e o monitoramento contínuo em tempo real. A primeira versão limita-se ao conjunto de verificações implementado e não pretende detectar todas as classes de vulnerabilidade existentes. Estas delimitações são retomadas na Seção 6.

## 2 REFERENCIAL TEÓRICO

### 2.1 Segurança de aplicações web e o custo do defeito tardio

Construir segurança *dentro* do software, em vez de acoplá-la ao final, é a tese central da disciplina de segurança de software (McGRAW, 2006; VIEGA; McGRAW, 2001). Essa perspectiva contrapõe-se ao modelo em que a segurança é tratada como uma etapa de auditoria posterior à construção. O argumento econômico que a sustenta é clássico: o custo de remoção de um defeito cresce de forma acentuada conforme ele progride pelas fases do ciclo de vida (BOEHM, 1981). Transposto para a segurança, isso significa que uma falha detectada durante o desenvolvimento é incomparavelmente mais barata de corrigir do que a mesma falha explorada em produção. O movimento *DevSecOps* e a prática de *shift-left security* operacionalizam essa ideia, integrando verificações de segurança automatizadas ao fluxo contínuo de desenvolvimento e entrega (SHOSTACK, 2014).
...
Duas grandes abordagens automatizadas coexistem. O *Static Application Security Testing* (SAST) analisa o código-fonte sem executá-lo; o *Dynamic Application Security Testing* (DAST) analisa a aplicação em execução, observando e provocando seu comportamento pela interface externa, sem necessariamente acessar o código (BAU et al., 2010). Cada abordagem tem pontos cegos complementares: o SAST enxerga o código mas não o comportamento em tempo de execução, ao passo que o DAST enxerga o comportamento mas não a causa no código. O SentinelScope situa-se na abordagem DAST, coerente com a delimitação de não pressupor acesso ao repositório.

A eficácia dos scanners DAST de caixa-preta, contudo, é limitada e desigual. Doupé, Cova e Vigna (2010), em avaliação que ficou conhecida pelo título "Why Johnny Can't Pentest", demonstraram que scanners automáticos falham sistematicamente ao lidar com o estado da aplicação e deixam de alcançar partes relevantes da superfície de ataque. Bau et al. (2010) e Fonseca, Vieira e Madeira (2007) documentaram ampla variação de cobertura entre ferramentas e a coexistência de falsos negativos e falsos positivos, especialmente em injeção de SQL e XSS. Antunes e Vieira (2010) propuseram *benchmarks* para tornar essa comparação metodologicamente rigorosa. A lição para este trabalho é dupla: (i) nenhuma ferramenta automatizada deve ser apresentada como garantia de ausência de vulnerabilidades — o que fundamenta o posicionamento de *apoio*; e (ii) a redução de falsos positivos e a qualificação da confiança de cada achado não são refinamentos opcionais, mas requisitos centrais de utilidade.

### 2.4 O fator humano: usabilidade das ferramentas de segurança

A terceira base teórica desloca o foco da máquina para a pessoa. Mesmo ferramentas tecnicamente competentes fracassam quando seus resultados não são acionáveis. Johnson et al. (2013), em estudo qualitativo com desenvolvedores, concluíram que a subutilização de ferramentas de análise decorre principalmente de dois fatores: o volume de falsos positivos e a forma como os resultados são comunicados — sem explicação suficiente e sem integração com o fluxo de trabalho. Bessey et al. (2010), relatando anos de uso industrial de análise estática, reforçam que a confiança do desenvolvedor no resultado é um recurso frágil: poucos falsos positivos ruidosos bastam para que a ferramenta seja abandonada. Smith et al. (2015) mapearam as perguntas que desenvolvedores fazem ao diagnosticar um alerta de segurança, revelando que a informação de que precisam raramente é a mesma que as ferramentas oferecem. Acar et al. (2016) mostraram, experimentalmente, que a *fonte* de informação usada pelo desenvolvedor influencia diretamente a segurança do código produzido — evidência de que orientação clara e correta embutida na ferramenta tem efeito prático mensurável. Green e Smith (2016) sintetizam esse conjunto em um princípio de projeto: "desenvolvedores não são o inimigo"; a segurança utilizável precisa encontrá-los onde eles estão, com linguagem e exemplos que eles compreendam.

Essa literatura fundamenta a hipótese de projeto do SentinelScope: **o valor de um achado de segurança para o desenvolvedor não está apenas em detectá-lo, mas em explicá-lo e torná-lo corrigível**. A separação entre severidade e confiança, a supressão de sinais instáveis e a apresentação em três camadas (o que se observou, por que importa, como corrigir) são respostas diretas às barreiras identificadas por Johnson et al. (2013) e Green e Smith (2016).

## 3 METODOLOGIA

### 3.1 Classificação da pesquisa

Quanto à natureza, esta é uma **pesquisa aplicada**, pois gera um artefato de software destinado a uso prático. Quanto aos objetivos, é **exploratória e descritiva**. Quanto ao procedimento, adota o método de **pesquisa de desenvolvimento (ou pesquisa-artefato)**, no qual o conhecimento é produzido pela construção e avaliação de um artefato tecnológico que responde a um problema real. A abordagem é predominantemente **qualitativa** na concepção e **quantitativa** na verificação (contagem de verificações, cobertura de testes e, como trabalho futuro, métricas de detecção).

### 3.2 Materiais e ambiente

A plataforma foi construída com PHP 8.2 (sem framework), Python 3.10 (sem dependências externas ao interpretador padrão), banco de dados MariaDB 10.4 e interface em HTML e CSS sem framework de front-end. A opção deliberada por não utilizar frameworks visou manter o código auditável e explícito, adequado a um trabalho acadêmico em que cada decisão precisa ser justificável, e reduzir a superfície de dependências de terceiros. O ambiente de desenvolvimento e teste utilizou a pilha XAMPP sobre Windows 11. O controle de versão foi feito com Git.
...
A separação entre a aplicação web (PHP) e o motor de varredura (Python) não é acidental: isola o componente que executa requisições potencialmente sensíveis em um processo com fronteira e orçamento próprios, e permite que o motor evolua e seja testado de forma independente da camada de apresentação.
...
Cada família de vulnerabilidade é implementada como uma classe de plugin que declara seus próprios metadados (categoria, CWE, OWASP, WSTG, tipo e risco operacional) e o conjunto de identificadores de verificação que possui. Duas modalidades coexistem:
...
Como ilustração do rigor do modo ativo, o plugin de injeção de SQL tenta as técnicas em ordem crescente de custo — baseada em erro, diferencial booleano e, por fim, baseada em tempo — e **para na primeira técnica que produz evidência confirmada**, sem jamais enumerar tabelas ou extrair registros. A técnica de diferencial booleano emprega o contraste entre uma condição logicamente verdadeira (`' AND '1'='1`), que deve manter a resposta equivalente à *baseline*, e uma condição falsa (`' AND '1'='2`), que deve divergir, acompanhado de um *probe* de controle inócuo — construção que evita o erro comum de usar `OR '1'='1'` sobre um parâmetro cujo valor de *baseline* já é verdadeiro, o que nunca divergiria e produziria conclusão inválida.

### 4.4 Confiança separada de severidade

Um dos contributos centrais do trabalho é tratar **severidade** e **confiança** como dimensões independentes. Severidade responde "quão grave seria isto se fosse verdadeiro"; confiança responde "quão seguro o detector está de que é verdadeiro". Um achado crítico com evidência fraca permanece *crítico de baixa confiança* — nunca é rebaixado silenciosamente para uma severidade menor para "compensar" a incerteza, prática que ocultaria riscos reais do desenvolvedor.
...
O resultado, limitado a 0–100, é mapeado para um *status* legível (*confirmado*, *alta confiança*, *provável*, *possível*, *informativo* ou *revisão manual necessária*). Por construção, verificações puramente passivas e de observação única não alcançam o *status* "confirmado", reservado a detecções ativas que reproduzem o resultado por uma técnica independente. Esse desenho é a resposta direta do trabalho à advertência de Bessey et al. (2010) e Johnson et al. (2013) sobre o custo dos falsos positivos: em vez de afirmar mais do que a evidência sustenta, a ferramenta comunica explicitamente seu grau de certeza e sinaliza o que requer revisão humana.

### 4.5 Redução de ruído e proporcionalidade

Antes de qualquer comparação entre respostas, a normalização remove o conteúdo volátil que, se ignorado, geraria diferenças espúrias e, portanto, falsos positivos. Sinais instáveis — por exemplo, um texto de erro presente em apenas uma de duas requisições de *baseline*, ou um valor de alta entropia que muda entre requisições (token rotativo) — são rebaixados a "revisão manual necessária" ou excluídos, em vez de confirmados. O contexto de página mantém a severidade proporcional: uma resposta que não é HTML não é penalizada por ausência de *Content-Security-Policy*, por exemplo. Essas decisões implementam, no plano técnico, o requisito de utilidade discutido na Seção 2.4.
...
O exemplo de correção é o elemento que distingue a explicabilidade proposta: em vez de descrever a correção em abstrato, a ferramenta a *mostra*. Cada uma das verificações que um plugin pode emitir possui um par de trechos autorado uma única vez em uma tabela central (`plugins/remediation_examples.py`) e mesclado a todo achado no momento de sua construção. Essa decisão responde diretamente às conclusões de Smith et al. (2015) sobre as perguntas que o desenvolvedor faz diante de um alerta — frequentemente "como eu conserto isto no meu código?" — e ao efeito, demonstrado por Acar et al. (2016), de que a fonte de informação embutida na ferramenta influencia a segurança do código produzido.

Abaixo dessas camadas, um painel recolhível de *contexto técnico* expõe o identificador da verificação, a taxonomia completa (categoria, CWE, OWASP, WSTG), a URL/parâmetro afetado e a evidência estruturada em JSON, para quem quiser aprofundar. Severidade e confiança são exibidas como selos distintos. Esse formato materializa o princípio de Green e Smith (2016): a informação de que o desenvolvedor precisa vem primeiro, em sua linguagem; o detalhe para o especialista fica disponível, mas subordinado.

### 4.7 Varredura autenticada

Boa parte da superfície de risco de uma aplicação só é acessível após autenticação. Para alcançá-la sem acoplar um mecanismo de *login* específico, o motor aceita cabeçalhos de autenticação (tipicamente um *cookie* de sessão ou um *token* Bearer) que são anexados a toda requisição — passiva ou ativa — no único ponto de saída HTTP autorizado. Por construção, esses cabeçalhos só trafegam para o host e a porta fixados pelo controle de escopo, são redigidos no registro de requisições e, na aplicação web, são usados de forma transitória em uma análise e nunca persistidos. Assim, uma única extensão localizada habilita a varredura autenticada para todas as verificações existentes de uma vez, sem alterar nenhum plugin.

### 4.8 Pontuação, histórico e comparação

Cada análise concluída recebe uma **Pontuação de Segurança** de 0 a 100, calculada por uma regra determinística: a pontuação parte de 100 e cada achado crítico, alto, médio ou baixo desconta, respectivamente, 30, 15, 7 e 2 pontos, com resultado limitado a 0–100. A regra é intencionalmente simples e reprodutível, adequada à *comparação* entre análises da mesma aplicação; não pretende ser uma medida universal de segurança — limitação assumida explicitamente na interface e nesta monografia. O histórico por aplicação e a comparação entre duas análises (classificando achados em *novos*, *ainda presentes* e *corrigidos*, por meio de uma impressão digital estável de cada achado) sustentam o uso incremental da ferramenta ao longo das entregas. Para integração com o fluxo de trabalho da equipe, a interface ainda permite filtrar os achados por severidade, copiar o exemplo de correção com um clique e exportar a análise concluída em JSON legível por máquina, adequado a *pipelines* de integração contínua e a sistemas de tíquetes.

### 4.9 Salvaguardas de segurança da aplicação

A própria aplicação foi construída sob as práticas que ela verifica: mutações exigem *token* CSRF vinculado à sessão; toda a saída passa por codificação para mitigar XSS armazenado; todo acesso ao banco usa *prepared statements* nativos, prevenindo injeção de SQL. A versão acadêmica assume instalação local monousuário confiável; autenticação e controle de acesso por papéis são recomendados antes de qualquer implantação compartilhada.

## 5 AVALIAÇÃO E DISCUSSÃO

### 5.1 Estratégia de verificação e resultados dos testes automatizados

A verificação funcional foi conduzida por uma suíte de **94 testes automatizados** em Python (framework `unittest`), complementada por **40 verificações no lado PHP**. A suíte cobre vários níveis: a fundação do motor (`test_engine.py`), cada plugin contra fixtures vulnerável e seguro (`test_plugins.py` e os testes por plugin ativo), a integração catálogo/orquestrador (`test_scanner.py`), a camada de exemplos de correção (`test_remediation_examples.py`), o arcabouço de avaliação empírica (`test_evaluation.py`), a varredura autenticada e o teste de formulários via POST (`test_authenticated_and_forms.py`), o retry do transporte em falhas transitórias (`test_transport.py`), e as classes puras do lado PHP — renderização, exemplos de código, cálculo de pontuação, comparação de análises e um benchmark de 1.000 achados (`php_test.php`). Na execução de referência, os 94 testes Python foram aprovados (resultado "OK"), em tempo total da ordem de 180 segundos — tempo dominado pelos testes de detecção baseada em tempo, que empregam atrasos reais na rede para exercitar a lógica temporal de forma fidedigna.
...
Os resultados sustentam a viabilidade técnica da proposta e, sobretudo, a viabilidade do seu diferencial: é possível construir, no escopo de um trabalho acadêmico, um motor DAST não destrutivo que qualifica a confiança de cada achado e reduz ruído por normalização e verificação de estabilidade. A cobertura de testes contra fixtures seguros — e não apenas vulneráveis — é a evidência de que a ferramenta foi projetada para *não gritar* diante de configurações corretas, requisito que a literatura aponta como decisivo para a adoção (JOHNSON et al., 2013; BESSEY et al., 2010).

Cabe, porém, distinguir com honestidade o que foi verificado do que ainda não foi. Os testes comprovam que cada verificação **funciona como especificado contra alvos controlados**. Eles não medem, ainda, o desempenho da ferramenta contra uma aplicação real e desconhecida — isto é, sua taxa de detecção (recall) e sua taxa de falsos positivos (precision) em condições não controladas. Essa é a fronteira entre o que este trabalho entrega e o que a Seção 5.3 propõe.
...
b) **Base de comparação (baseline).** OWASP ZAP em modo *baseline*, ferramenta livre amplamente utilizada, como referência de mercado;

c) **Métricas.** Para cada classe de vulnerabilidade conhecida no alvo, apurar verdadeiros positivos, falsos positivos e falsos negativos, derivando *precision*, *recall* e *F1*, além do tempo de execução e do número de requisições emitidas;
...
Reconhecem-se as seguintes limitações. **De construção:** o esquema de pontuação é uma heurística de comparação, não uma medida absoluta de segurança. **De cobertura:** o conjunto de verificações é finito e não detecta todas as classes de vulnerabilidade; falhas de lógica de negócio e vulnerabilidades que dependem de estado autenticado complexo estão além do alcance de um scanner black-box, conforme já demonstrado por Doupé, Cova e Vigna (2010). **De avaliação:** como discutido, a eficácia em alvos reais ainda não foi medida (Seção 5.3). **De generalização:** os testes automatizados usam alvos controlados construídos pelo próprio autor, o que exige a avaliação independente proposta para mitigar viés. Essas limitações não invalidam a contribuição, mas delimitam com precisão o alcance das conclusões — postura que a própria literatura de scanners recomenda como antídoto ao exagero de capacidade (BAU et al., 2010).

## 6 CONSIDERAÇÕES FINAIS

Este trabalho partiu de um problema concreto e recorrente — equipes de desenvolvimento sem apoio especializado em segurança, verificando suas aplicações de forma tardia e pontual — e de uma lacuna reconhecida na literatura: a distância entre a capacidade de *detectar* vulnerabilidades e a capacidade de o desenvolvedor *agir* sobre elas. A resposta construída, o SentinelScope, é uma plataforma DAST de apoio, não destrutiva, que ancora seus achados em taxonomias consolidadas (OWASP Top 10, CWE, WSTG), separa explicitamente severidade de confiança, reduz ruído por normalização e verificação de estabilidade, e — sobretudo — apresenta cada achado em uma camada de explicabilidade orientada à correção.
...
OWASP FOUNDATION. **OWASP ZAP — Zed Attack Proxy**. [S. l.]: OWASP. Disponível em: https://www.zaproxy.org/. Acesso em: 8 set. 2026.
