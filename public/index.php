...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($page==='run'){
        ...
        $result=$scannerClient->scan($app['base_url'],$mode);
...
if($page==='result'||$page==='report'){
    ...
    $buttons=$page==='report'?'<button onclick="print()">Imprimir / Salvar como PDF</button>':'<a class="button secondary" href="index.php?page=report&id='.$a['id'].'">Relatório para impressão</a>';
    ...
    if($a['status']==='failed')$body.='<div class="error"><b>A análise não pôde ser concluída.</b><br>'.e($a['error_message']).'</div>';else $body.='<div class="metrics"><div><span>Verificações executadas</span><b>'.(int)$a['checks_run'].'</b></div><div><span>Achados</span><b>'.count($rows).'</b></div><div><span>Status HTTP</span><b>'.(int)$a['http_status'].'</b></div><div><span>Duração</span><b>'.(int)$a['duration_ms'].' ms</b></div></div><h2>O que sua equipe deve corrigir</h2>'.($list !== '' ? $list : '<div class="empty">Nenhum problema foi detectado pelas verificações configuradas.</div>').'<section class="limitations"><h2>Escopo e limitações</h2><p>'.($modeLabel==='Passiva'?'Esta é uma avaliação passiva: apenas observação de respostas, sem envio de payloads de teste.':'Esta análise incluiu testes ativos com payloads seguros e não destrutivos.').' Ela não prova a ausência de vulnerabilidades e não substitui um teste de intrusão profissional.</p></section>';
    Layout::render($page==='report'?'Relatório de segurança':'Resultado da análise',$body);exit;
}
if($page==='compare'){
    $old=$analysisRepository->find((int)($_GET['old']??0));$new=$analysisRepository->find((int)($_GET['new']??0));if(!$old||!$new||$old['application_id']!==$new['application_id'])throw new InvalidArgumentException('Escolha análises da mesma aplicação.');$left=array_column($findingRepository->byAnalysis((int)$old['id']),null,'fingerprint');$right=array_column($findingRepository->byAnalysis((int)$new['id']),null,'fingerprint');
    ...
    Layout::render('Comparação','<div class="toolbar"><a href="index.php">← Painel</a></div><h1>O que mudou?</h1><p>Análise #'.$old['id'].' → #'.$new['id'].' · Pontuação '.$old['security_score'].' → '.$new['security_score'].'</p><div class="comparison">'.$section('Novos problemas',array_diff_key($right,$left),'new').$section('Ainda presentes',array_intersect_key($right,$left),'same').$section('Corrigidos',array_diff_key($left,$right),'fixed').'</div>');exit;
}
