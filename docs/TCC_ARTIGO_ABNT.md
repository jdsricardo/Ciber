...
- **Rastreamento** (`engine/crawler.py`): expansão de uma única URL-semente para a superfície alcançável de mesma origem, seguindo *links* `<a>`, alvos de formulários GET e redirecionamentos, de modo que a varredura cubra páginas que o usuário não informou diretamente (Seção 4.7);
...
- **Orquestrador** (`engine/runner.py`): conduz o rastreamento em largura e, para cada página descoberta, coleta as requisições de *baseline*, constrói o contexto de página e executa cada plugin registrado; consolida os achados entre páginas (condições de servidor, como cabeçalhos ausentes, uma vez por identificador; achados de injeção e exposição por identificador, URL e parâmetro) e não contém lógica específica de nenhuma vulnerabilidade.
...
### 4.7 Rastreamento da superfície e varredura autenticada

Uma varredura que testasse apenas a URL informada deixaria de fora a maior parte de uma aplicação. Um exemplo torna a limitação concreta: se o usuário aponta a ferramenta para uma tela de *login*, mas existe uma página desprotegida a um salto de distância, essa página nunca seria examinada. Para eliminar essa lacuna, o motor realiza um **rastreamento em largura** (`engine/crawler.py`) a partir da URL-semente. Ele segue os *links* `<a>`, os alvos de formulários GET e os redirecionamentos que permaneçam na mesma origem (host e porta fixados), e examina integralmente cada página descoberta — as duas requisições de *baseline* e todos os plugins — antes de enfileirar seus próprios *links*. Assim, a ferramenta descobre e testa por conta própria páginas que não foram informadas.

O rastreamento é deliberadamente **somente-leitura** e limitado. Nunca segue *links* de *logout* nem *links* que alterem estado (por exemplo, `?action=delete` ou `?remove=`), de modo que uma sessão autenticada sobrevive ao rastreamento e a própria navegação não dispara nenhum efeito colateral destrutivo. Páginas equivalentes para fins de teste — mesma rota e mesmo conjunto de nomes de parâmetros, ignorando os valores — são visitadas uma única vez, o que impede que um catálogo do tipo `/produto?id=1..N` exploda a varredura. Um teto de páginas e de profundidade, somado ao orçamento único de requisições e de tempo compartilhado com a fase de *probes*, garante que a varredura jamais ultrapasse seu envelope de segurança, por maior que seja o alvo.

Boa parte da superfície de risco, porém, só fica acessível após a autenticação. Para alcançá-la sem acoplar um mecanismo de *login* específico, o motor aceita cabeçalhos de autenticação, em geral um *cookie* de sessão ou um *token* Bearer, e os anexa a toda requisição, passiva ou ativa, no único ponto de saída HTTP autorizado. Por construção, esses cabeçalhos só trafegam para o host e a porta fixados pelo controle de escopo e são ocultados no registro de requisições. Na aplicação web, eles são usados de forma transitória em uma única análise e nunca ficam armazenados. Rastreamento e autenticação combinam-se de forma natural: com o *cookie* de sessão anexado, o rastreador percorre a área autenticada — descobrindo e examinando páginas que só existem após o *login* — sem, ainda assim, seguir os *links* de encerramento de sessão. Dessa forma, uma extensão pequena e localizada habilita a varredura autenticada e em profundidade para todas as verificações de uma vez, sem alterar nenhum plugin.
...
Cabe, porém, distinguir com honestidade o que foi verificado do que ainda não foi. Os testes automatizados comprovam que cada verificação funciona como especificado contra alvos controlados. A esse resultado soma-se agora uma **execução preliminar contra um alvo real e público** (Seção 5.3.1), que forneceu uma primeira medida de precisão e revocação e, mais do que os números, expôs de forma verificável tanto um acerto do desenho (o rastreamento alcançou vulnerabilidades fora da página-semente; a travessia foi emitida como "revisão manual") quanto duas fragilidades de causa conhecida (um falso negativo por formulário com *action* em caminho distinto e um falso positivo do detector NoSQL diante de um 404 estável). O que permanece por medir é o desempenho comparado, em maior escala e contra uma *baseline* de mercado, sob as classes e contextos mais variados de um laboratório dedicado. Essa é a fronteira entre o que este trabalho entrega e o que a Seção 5.3.2 propõe.

### 5.3 Avaliação empírica

Para elevar a evidência de "funciona como especificado" a "detecta com que eficácia", esta seção reúne duas partes: uma **execução preliminar já realizada** contra um alvo público autorizado (Seção 5.3.1) e a **proposta de avaliação comparativa** de maior escala que permanece como trabalho de continuidade (Seção 5.3.2). A separação é deliberada e responde à recomendação de não exagerar a capacidade da ferramenta (BAU et al., 2010): reporta-se com rigor o que foi medido e distingue-se com clareza o que ainda falta medir.

#### 5.3.1 Execução preliminar em alvo público autorizado

Um laboratório completo com *Damn Vulnerable Web Application* (DVWA), *OWASP Juice Shop* e *OWASP WebGoat*, além da *baseline* OWASP ZAP, exige um ambiente com conteinerização e execução de serviços que não estava disponível no equipamento de desenvolvimento. Para obter, ainda assim, uma primeira medida real de eficácia, executou-se o SentinelScope contra o **Altoro Mutual** (`demo.testfire.net`), aplicação bancária deliberadamente vulnerável mantida pela IBM e disponibilizada publicamente para teste de ferramentas de segurança. A varredura foi ativa (*safe-active*), partindo apenas da raiz do site, e o rastreamento (Seção 4.7) alcançou por conta própria as demais páginas.

O procedimento seguiu a metodologia de Fonseca, Vieira e Madeira (2007): para cada família no escopo, estabeleceu-se o *ground-truth* — se a vulnerabilidade está de fato presente na superfície não autenticada — por **verificação manual**, reproduzindo cada requisição e confirmando o comportamento do alvo. A apuração de verdadeiros positivos (VP), falsos positivos (FP) e falsos negativos (FN) e o cálculo de precisão, revocação e F1 foram automatizados por um arcabouço reproduzível (`evaluation/evaluate.py`), que se recusa a executar sem uma declaração explícita de autorização do alvo. O Quadro 2 apresenta o resultado.

Quadro 2 — Resultado da execução preliminar sobre o Altoro Mutual (superfície não autenticada, alcançada por rastreamento; 909 requisições).

| Classe (CWE) | Presente no alvo | VP | FP | FN | Precisão | Recall | F1 |
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
...
Mais valioso que os números agregados é a **análise das duas falhas**, que confirma a honestidade do desenho e aponta melhorias concretas:

- **Falso negativo (injeção de SQL).** O formulário de *login* do Altoro é comprovadamente injetável — o valor `' OR '1'='1` no campo `uid` produz um erro HTTP 500, sinal de que a entrada alcança a consulta SQL. O SentinelScope não o reportou, e a causa é precisa: o formulário tem `action="doLogin"`, um caminho diferente do da página que o contém, ao passo que, nesta versão, o *probe* de formulário é submetido à própria URL da página (limitação já registrada na Seção 4.3). O *probe*, portanto, nunca alcança o manipulador vulnerável. É um falso negativo de causa conhecida, e não um erro de detecção da técnica de SQL.

- **Falso positivo (injeção NoSQL).** O detector NoSQL sinalizou o parâmetro `content` em `default.jsp`. A verificação manual mostrou que essa página responde com um HTTP 404 estável e idêntico tanto para o operador `[$ne]` quanto para `[$eq]` e para a linha de base — ou seja, o parâmetro não é processado. O detector interpretou a estabilidade do 404 como um "sucesso de contorno de operador". A correção indicada é exigir que a resposta de controle seja de fato distinta de uma página de erro genérica antes de confirmar o achado.

Reforça-se que o achado de XSS refletido só foi possível porque o rastreamento levou a varredura a páginas que não eram a semente (o parâmetro `content` em `index.jsp` e o campo `query` em `search.jsp`), e que a travessia de diretório foi corretamente emitida com *status* "revisão manual necessária", sem afirmar mais do que a evidência sustentava. Ambos os comportamentos são exatamente os pretendidos pelo desenho da ferramenta.

#### 5.3.2 Proposta de avaliação comparativa (trabalho de continuidade)

A execução preliminar mede o SentinelScope de forma isolada e em um único alvo. Para uma evidência de eficácia mais robusta e generalizável, propõe-se ampliá-la, seguindo a tradição metodológica de Fonseca, Vieira e Madeira (2007), Bau et al. (2010) e Antunes e Vieira (2010):

a) **Alvos.** Aplicações deliberadamente vulneráveis de referência — DVWA, OWASP Juice Shop e OWASP WebGoat — executadas em laboratório isolado e autorizado, ampliando as classes de vulnerabilidade e os contextos avaliados;
...
c) **Métricas.** As mesmas do Quadro 2 (VP, FP, FN, precisão, revocação e F1), acrescidas do tempo de execução e do número de requisições emitidas, agora lado a lado com a *baseline*;
...
O Quadro 3 apresenta a estrutura de coleta comparativa a ser preenchida com os dados dessa etapa. O arcabouço `evaluation/evaluate.py`, já exercitado na Seção 5.3.1, torna a coleta da coluna do SentinelScope imediata; resta acoplar a coluna da *baseline*.

Quadro 3 — Modelo de tabulação da avaliação comparativa proposta (a preencher).
...
Reconhecem-se cinco limitações. A primeira é de construção: o esquema de pontuação é uma heurística de comparação, não uma medida absoluta de segurança. A segunda é de cobertura: o conjunto de verificações é finito e não detecta todas as classes de vulnerabilidade. Falhas de lógica de negócio e vulnerabilidades que dependem de estado autenticado complexo estão além do alcance de um scanner de caixa-preta, como já demonstraram Doupé, Cova e Vigna (2010). A terceira é de avaliação: como discutido, a eficácia em alvos reais ainda não foi medida de forma controlada (Seção 5.3). A quarta é de generalização: os testes automatizados usam alvos controlados construídos pelo próprio autor, o que torna necessária a avaliação independente proposta para reduzir o viés. A quinta é de rastreamento: o rastreador analisa o HTML retornado pelo servidor e segue *links* e redirecionamentos, mas não executa JavaScript. Em aplicações de página única (SPA), cuja navegação é construída dinamicamente no cliente, a superfície descoberta é menor — comportamento observado em um dos alvos de teste, cujo rastreamento se restringiu às poucas rotas presentes no HTML inicial. Essas limitações não invalidam a contribuição, mas delimitam com precisão o alcance das conclusões. Essa é, aliás, a postura que a própria literatura de scanners recomenda como antídoto ao exagero de capacidade (BAU et al., 2010).
...
A principal contribuição não está em detectar mais do que as ferramentas existentes, mas em **comunicar melhor** o que se detecta, atacando a barreira de usabilidade que a literatura identifica como causa central da subutilização das ferramentas de segurança (JOHNSON et al., 2013; GREEN; SMITH, 2016). Reforçando essa contribuição, a camada de explicabilidade foi ampliada com exemplos de código "vulnerável → corrigido" por verificação (Seção 4.6), a varredura passou a rastrear a superfície alcançável de mesma origem — descobrindo e testando páginas que não foram informadas, inclusive em áreas autenticadas por cabeçalho de sessão (Seção 4.7) — e o teste de parâmetros passou a cobrir campos de formulário via POST nos detectores de maior valor (Seção 4.3). Como continuidade, destacam-se: (i) a ampliação da avaliação empírica preliminar já executada (Seção 5.3.1) para o formato comparativo proposto na Seção 5.3.2, com métricas de *precision/recall/F1* contra os alvos de referência DVWA, Juice Shop e WebGoat e uma baseline de mercado; (ii) a extensão do teste via POST aos demais detectores ativos; (iii) a automação de sequências de *login* por formulário (submissão de credenciais e captura do *cookie* resultante) e OAuth, além do cabeçalho de sessão e do rastreamento autenticado já suportados; (iv) o rastreamento de aplicações de página única por meio da execução de JavaScript em um navegador controlado; e (v) a introdução de autenticação e controle de acesso por papéis para uso multiusuário da própria ferramenta. Conclui-se que o tema é relevante, plausível e viável como trabalho de conclusão de curso, e que o artefato produzido oferece base sólida tanto para a defesa acadêmica quanto para evolução prática.
